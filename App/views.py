...
def viewcart(request):
    c=Cart.objects.filter(uid=request.user.id) # cart info of a particular user id
    context={}
    context['data']=c
    sum=0
    for x in c:
...
def placeorder(request):
    ...
    for x in c:
        ...
        o=Order.objects.create(orderid=orderid,pid=x.pid,uid=x.uid,amt=amount)
        o.save()
...
def  makepayment(request):
    client = razorpay.Client(auth=("rzp_test_nfGK5rWT6zZMJU", "fo8yb8yDeA5ore6WJs0bqAbF"))
    o=Order.objects.filter(uid=request.user.id)
    sum=0
    for x in o:
        sum=sum+x.amt
...
def paymentsuccess(request):
    sub = 'Dog Shop Order Status'
    msg = 'Thanks For Buying...!!'
    frm = 'chintalaanudeep41@gmail.com'
    u = User.objects.filter(id=request.user.id)
    to = u[0].email
    send_mail(
        sub,
